...
mod tests {
    ...
    #[test]
    fn test_validate_total_parts() {
        assert!(validate_total_parts(1).is_ok());
        assert!(validate_total_parts(100).is_ok());
        assert!(validate_total_parts(0).is_err());
        assert!(validate_total_parts(20000).is_err());
    }

    #[test]
    fn test_validate_original_size() {
        assert!(validate_original_size(1024).is_ok());
        assert!(validate_original_size(1024 * 1024 * 1024).is_ok());
        assert!(validate_original_size(0).is_err());
        assert!(validate_original_size(100 * 1024 * 1024 * 1024).is_err());
    }

    #[test]
    fn test_validate_pe_format() {
        ...
        assert!(validate_pe_format(&pe_data).is_ok());

        let invalid_data = vec![0u8; 256];
        assert!(validate_pe_format(&invalid_data).is_err());
    }

    #[test]
    fn test_is_likely_binary() {
        let pe_data = b"MZ\x90\x00\x03\x00\x00\x00";
        assert!(is_likely_binary(pe_data));
    ...
    fn test_validate_data_size() {
        ...
        assert!(validate_data_size(&header, &data).is_ok());
    ...
    #[test]
    fn test_validate_data_checksum() {
        let data = vec![0u8; 100];
        let checksum = calculate_checksum(&data);
        let header = PartHeader::new(0, 1, 100, 100, checksum, checksum);

        assert!(validate_data_checksum(&header, &data).is_ok());

        let wrong_checksum = [0xff; 32];
        let bad_header = PartHeader::new(0, 1, 100, 100, wrong_checksum, checksum);
        assert!(validate_data_checksum(&bad_header, &data).is_err());
    }
    ...
    fn test_validate_part() {
        ...
        assert!(validate_part(&header, &data).is_ok());
    }

    #[test]
    fn test_validate_final_checksum() {
        let data = b"Test data for checksum";
        let checksum = calculate_checksum(data);

        assert!(validate_final_checksum(data, &checksum).is_ok());

        let wrong_checksum = [0u8; 32];
        assert!(validate_final_checksum(data, &wrong_checksum).is_err());
    }
}
